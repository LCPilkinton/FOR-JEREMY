...
#Lisa Pilkinton
#5/2/26 Species included with a random slope and random intercept looking at plant functional grouping only
...
PFUNC_SP_RAN_SL_INT <- brm( #I use bf() to set up several formula
  # the cover model
  bf(PropCov ~ 0 + YEAR_relative_sc * PFUNC + (1 + YEAR_relative_sc | SiteSubsite:PFUNC) + (1|SiteSubsitePlot:PFUNC)+ (1 + YEAR_relative_sc |SPECIES_NAME:PFUNC),
     # the absence presence model
     zi~ 0 + YEAR_relative_sc * PFUNC + (1 + YEAR_relative_sc | SiteSubsite:PFUNC) + (1|SiteSubsitePlot:PFUNC)+ (1 + YEAR_relative_sc |SPECIES_NAME:PFUNC) ),
  data = MYC_ARCTIC_CTL,
  ...
  file = "PFUNC_SP_RAN_SL_INT"   # <-- autosaves every few minutes
)
