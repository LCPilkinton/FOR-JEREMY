...
model3_brms_JB_plot <- brm( #I use bf() to set up several formula
  ...
  bf(PropCov ~ 0 + YEAR_relative_2 * GMYCPFUNC + (1 + YEAR_relative_2 | SiteSubsite:GMYCPFUNC) + (1|SiteSubsitePlot:GMYCPFUNC),
     ...
     zi~ 0 + YEAR_relative_2 * GMYCPFUNC + (1 + YEAR_relative_2 | SiteSubsite:GMYCPFUNC) + (1|SiteSubsitePlot:GMYCPFUNC) ),
  data = MYC_ARCTIC_CTL,
 ...
 # init = 0,
  ...
  file = "model3_brms_JB_plotid"   # <-- autosaves every few minutes
)
library(data.table)
MYC_ARCTIC_CTL_dt <- data.table(MYC_ARCTIC_CTL)

dummy_dt <- MYC_ARCTIC_CTL_dt[,.(YEAR_relative_2 = range(YEAR_relative_2)),by = .(SiteSubsite,GMYCPFUNC,GMYC,PFUNC)]
dummy_dt[,diff_year := diff(YEAR_relative_2),by = .(SiteSubsite,GMYCPFUNC)]

preds <- fitted(model3_brms_JB_plot,
                   newdata = dummy_dt,
                   re_formula  =  ~  (1 + YEAR_relative_2 | SiteSubsite:GMYCPFUNC)  ,
                   seed = 0,allow_new_levels = F,summary = F)
preds <- preds*100 # just to get a cover 100
preds <- t(preds)
preds <- cbind(dummy_dt,preds)

preds <- data.table(melt(preds,id.vars = c("SiteSubsite","GMYCPFUNC","YEAR_relative_2","GMYC","PFUNC","diff_year")))  # prob_clim_dist

preds_change <- preds[,.(change = diff(value)/ unique(diff_year)  ),by = .(SiteSubsite,GMYCPFUNC,GMYC,PFUNC,variable)]


preds_change_summary <- preds_change[,.(mean = mean(change),
                                                   Q05 = quantile(change,probs = c(0.025)),
                                                   Q95 = quantile(change,probs = c(0.975))),by = .(SiteSubsite,GMYCPFUNC,GMYC,PFUNC)][order(-mean),]
preds_change_summary[,signif := ifelse(sign(Q05) == sign(Q95) & sign(mean) == -1,"Decline",
                                             ifelse(sign(Q05) == sign(Q95) & sign(mean) == 1,"Increase","Non signif"))]
table(preds_change_summary$signif,preds_change_summary$GMYCPFUNC)
table(preds_change_summary$mean>0)
...
fitmod3 <- model3_brms_JB_plot
summary(fitmod3) #this looks good so far

plot(fitmod3) # all good

pp_check(fitmod3, ndraws = 20) # this looks okay
