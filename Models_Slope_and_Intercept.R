...
#now I make species model, then GMYCPFUNC on Species

#SPECIES_RANDOM <- brm( #I use bf() to set up several formula
# the cover model
# bf(PropCov ~ 0 + YEAR_relative_sc * GMYCPFUNC + (1 + YEAR_relative_sc | SiteSubsite:GMYCPFUNC) + (1|SiteSubsitePlot:GMYCPFUNC)+ (1|SPECIES_NAME:GMYCPFUNC),
# the absence presence model
#     zi~ 0 + YEAR_relative_sc * GMYCPFUNC + (1 + YEAR_relative_sc | SiteSubsite:GMYCPFUNC) + (1|SiteSubsitePlot:GMYCPFUNC)+ (1|SPECIES_NAME:GMYCPFUNC) ),
#  data = MYC_ARCTIC_CTL,
...
# file = "SPECIES_RANDOM"   # <-- autosaves every few minutes
#)



SPECIES_RANDOM_SLOPE <- brm( #I use bf() to set up several formula
  # the cover model
  bf(PropCov ~ 0 + YEAR_relative_sc * GMYCPFUNC + (1 + YEAR_relative_sc | SiteSubsite:GMYCPFUNC) + (1|SiteSubsitePlot:GMYCPFUNC)+ (1 + YEAR_relative_sc |SPECIES_NAME:GMYCPFUNC),
     # the absence presence model
     zi~ 0 + YEAR_relative_sc * GMYCPFUNC + (1 + YEAR_relative_sc | SiteSubsite:GMYCPFUNC) + (1|SiteSubsitePlot:GMYCPFUNC)+ (1 + YEAR_relative_sc |SPECIES_NAME:GMYCPFUNC) ),
  data = MYC_ARCTIC_CTL,
  family = zero_inflated_beta(),
  prior = my_prior,  
  chains = 3,
  ...
  # control = list(adapt_delta = 0.99), # this increase the computation time like crazy, and often better model specification can fix the issues
  #  backend = "cmdstanr", # you can comment this or install cmdstanr, more stable API <- wouldn't install on my R
  file = "SPECIES_RANDOM_SLOPE"   # <-- autosaves every few minutes
)
